/*
* chardev_1.c: Crea un solo un dispositivo de lectura/escritura de caracteres que 
* 
*/
...
#define DEVICE_NAME "chardev_1" /* Dev nombre tal como aparece en /proc/devices */
...
static ssize_t device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	// Variable buffer local para guardar mensaje ingresado por
        // usuario en buffer global (msg[]) del modulo
	char msg_local[BUF_LEN];
	...
	// guardamos cada caracter escrito en buff( en modo usuario) hacia msg_local[](en modo kernel)
	for (i = 0; i < len && i < BUF_LEN; i++)
                get_user(msg_local[i], buff + i);
	
	/* Agregamos el fin de cadena '\0' */
	msg_local[len] = '\0';
	
	// Escribimos formato de mensaje en buffer global del modulo (msg[])
	sprintf(msg, "\nSe escribio por %d° vez!!!\nSu excritura fue: %s\n", counter++,msg_local);
